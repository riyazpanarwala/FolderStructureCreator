...
### 📊 Blueprint Architecture Metrics Dashboard (`Ctrl + M`)
...
- **Org Chart Diagram** — Interactive dendrogram diagram with depth-colored boxes and right-angle connector lines.
  - **Compact & Dynamic Box Sizing** — Vertically streamlined boxes (26px height) with reduced top/bottom padding and strictly centered text. Leaf boxes adapt dynamically to content length with reasonable minimum padding for a clean, compact look, while parent/category boxes maintain standard width to clearly display the hierarchy and connector lines.
  - **Guaranteed Collision-Free Layout** — Recursive subtree layout engine ensures no two boxes ever overlap horizontally or vertically regardless of depth, folder count, or dimension changes, preserving clean spacing between adjacent boxes and aligning connector lines.
  - **Fast Initial Chart Load (Capped at 100 Folders)** — When opening or importing large folder structures, the chart automatically expands level-by-level up to the first 100 folders to keep rendering blazing fast and responsive. Deeper branches beyond 100 start collapsed with `+Count` badges that users can expand individually or all at once via the **Expand all** button (<kbd>Ctrl+Shift+E</kbd>).
  ...
  - **5 Diagram Layout Modes** — Switch seamlessly between layout formats via the diagram sub-toolbar button, context menu, or Spotlight Command Palette (<kbd>Ctrl+K</kbd>):
    - **Horizontal (Left-to-Right ➡️)** — Classic dendrogram with dynamic column sizing, ideal for deep paths.
    - **Vertical (Top-to-Bottom ⬇️)** — Traditional top-down organizational tree structure.
    - **Mindmap (Bilateral Balance ☵)** — Central root hub with children dynamically distributed across mirrored left and right wings.
    - **Radial (360° Circular Orbit ⭕)** — Concentric circular orbital layers distributing subtrees proportionally across a full 360° circle.
    - **Sunburst (Multi-level Rings ☀️)** — Visual hierarchical partition disk with concentric annular sectors proportionally weighted by descendant folder count.
  - **🔀 Connector Line Styles** — Switch between 3 diagram connection styles via toolbar button, context menu, or Command Palette:
    ...
    *(Fully reflected across all layouts, live canvas rendering, Fullscreen Meeting Mode, and PNG/SVG/PDF exports).*
  ...
  - **Double-Click to Open in Explorer** — Double-click any folder node in Tree View, box on the diagram canvas, or annular sector in Sunburst to open its physical location in Windows Explorer.
